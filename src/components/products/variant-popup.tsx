...
export function VariantPopup({ product, variants, slug }: { product: { name: string; slug: string }; variants: ProductVariant[]; slug: string }) {
  ...
  const isOut = (v: ProductVariant) => {
    const s = (v as { stock_quantity?: string | number | null }).stock_quantity;
    const n = s === "" || s == null ? null : Number(s);
    return n != null && Number.isFinite(n) && n <= 0;
  };
  const selOut = sel ? isOut(sel) : false;
          ...
          <>
            ...
            <div className="fixed inset-x-4 top-1/2 z-50 mx-auto max-w-md -translate-y-1/2 rounded-2xl bg-white p-6 shadow-2xl">
                ...
                {variants.map((v) => {
                  ...
                  return (
                    <button
                      ...
                      disabled={out}
                      className={`flex w-full items-center justify-between rounded-xl border px-4 py-3 text-left ${out ? "opacity-50 cursor-not-allowed bg-zinc-50" : selected === v.id ? "border-brand-600 bg-brand-50" : "border-zinc-200 hover:bg-zinc-50"}`}
                    >
                      <span className="font-semibold text-zinc-900">{v.name} {out && <span className="ml-2 rounded-full bg-red-100 px-2 py-0.5 text-xs font-bold text-red-600">Out of Stock</span>}</span>
              ...
              <div className="mt-6 flex gap-3">
                ...
                {selOut ? (
                  <span className="flex flex-1 items-center justify-center rounded-full bg-zinc-100 px-6 py-3 text-sm font-semibold text-zinc-500">Out of Stock</span>
                ) : (
